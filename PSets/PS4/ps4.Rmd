...
fvs <- read_csv("~/problem_sets/data/FVS2011-hands.csv")
```
...
fvs %>% 
  ...
  labs(x = "Age in Yrs",
       y = "Time looking at hands in s")

```

What do you conclude from this pattern of data?

> There may be a relationship between age and time spent looking at faces but further steps need to be taken to answer that.

What statistical analyses would you perform here to quantify these differences?

> A linear mixed effects model is one option (hands.look ~ age * condition (1|subject))
...
```{r}
randdf <- for($(1:10000) 
  in ${1:rnorm(mean = 0, sd = 1, n = 30)}) ${$p < .05$}}
